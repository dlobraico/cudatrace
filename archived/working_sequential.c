...
struct vec3 trace(struct ray ray, int *depth, int *isReflect, struct reflectdata *Rdata);
struct vec3 shade(struct sphere *obj, struct spoint *sp, int *depth, int *isReflect, struct reflectdata *Rdata);
...
void render(int xsz, int ysz, u_int32_t *fb, int samples) {
	...
	for(j=0; j<ysz; j++) {
		for(i=0; i<xsz; i++) {
			...
			for(s=0; s<samples; s++) {
				struct vec3 col = trace(get_primary_ray(i, j, s), depth, isReflect, RData);
                printf("i: %i\t", i);
                printf("j: %i\t", j);
                printf("s: %i\n", s);
		...
		while (isReflect[0])        //while there are still reflection rays to trace
        {
            ...
            rcol = trace(RData->r, depth, isReflect, RData);    //trace a reflection ray
...
//	struct sphere *iter = obj_list->next;
	...
	int iterincr = 0;
		struct sphere *iter = obj_list_flat;
