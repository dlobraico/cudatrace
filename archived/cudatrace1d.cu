...
int main(int argc, char **argv) {
    ...
    if(!(host_fb = (u_int32_t *)malloc(xres * yres * sizeof(u_int32_t)))) {
    ...
    start_time = get_msec();
    render1(xres, yres, host_fb, rays_per_pixel);
    ...
    for(i=0; i<yres; i++) {
        for(j=0; j<xres; j++) {
            fputc((host_fb[i + sizeof(u_int32_t)*j] >> RSHIFT) & 0xff, outfile);
            fputc((host_fb[i + sizeof(u_int32_t)*j] >> GSHIFT) & 0xff, outfile);
            fputc((host_fb[i + sizeof(u_int32_t)*j] >> BSHIFT) & 0xff, outfile);
            printf("%i, %i: %i\n", i, j, host_fb[i + sizeof(u_int32_t)*j]);
        }
    }
    fflush(outfile);
